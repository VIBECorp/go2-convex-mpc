"""
Demo 08: Standing posture control from a randomized initial stance

Each foot starts displaced from its nominal (default-stand) xy position by a
random offset drawn uniformly from +/- 30 mm, computed via per-leg inverse
kinematics. The MPC uses the actual foot positions (lever arms) so no
controller change is needed; this demo shows posture control remains stable
on the asymmetric support polygon:
    0-3 s   : hold neutral posture
    3-6 s   : pitch nod        (+/- 0.15 rad)
    6-9 s   : roll sway        (+/- 0.15 rad)
    9-12 s  : height squat     (+/- 40 mm)
    12-16 s : y body sway      (+/- 30 mm, 0.25 Hz)

Set the environment variable EX08_FOOT_SEED to an integer for reproducible
...
SEED = os.environ.get("EX08_FOOT_SEED")
...
# MPC cost: same posture-control weights as ex05
...
def solve_leg_ik(kin: PinGo2Model, leg: str, p_target_base, q_leg_init,
...
def sample_random_stance():
    ...
    """
    kin = PinGo2Model()                       # dedicated model, base at identity
...
ax.set_xlabel("x [m]")
ax.set_ylabel("y [m]")
ax.set_title("Foot placements (top view), offsets uniform +/- 30 mm")
ax.axis("equal")
ax.grid(True)
...
fig.suptitle("Posture Tracking on a Randomized Stance")
fig.tight_layout()
plt.show(block=False)
